...
Seven built-in themes. Each strip shows the theme's palette in the order the colours land on the bar: **directory · branch · model · Opus accent · additions · warnings/cost · removals · pacing marker**.
...
`default` follows your terminal's own ANSI palette (the swatch shows a representative set), so it tracks whatever colour scheme your terminal uses. `mono` emits no colour at all, and **any** theme degrades to plain text when [`NO_COLOR`](https://no-color.org/) is set. (Swatches are generated by `docs/assets/themes/generate-swatches.js` from the palettes in `apply_theme()`.)
...
```bash
colour_theme=tokyo-night    # default | nord | dracula | solarized | tokyo-night | catppuccin | mono
```
