...
- **Opt-in styling (v2.15.0)** — All default-off. `bar_gradient` colours the filled progress-bar cells with a **smooth per-cell** gradient: each cell's RGB is linearly interpolated across the theme's four ramp stops (the hexes are stashed in `RAMP_HEX` by `apply_palette()`/the default theme), truecolour per cell with a nearest-256 fallback (pure-bash integer lerp, no forks). v2.16.0 replaced the original 4-band version. No-op under mono/NO_COLOR. `nerd_font` swaps the scattered Unicode segment icons (`branch_icon`, `model_icon`, etc.) for Nerd Font glyphs defined once via `printf -v` (raw UTF-8 bytes — Bash 3.2-safe, no `\u`, fork-free, built only when enabled). `--demo [theme|all]` previews themes by re-invoking the script per theme with a `STATUSLINE_THEME` env override and the `--benchmark` canned payload — no recursion, no conf changes. The env value is captured into `_env_theme` *before* the conf is sourced and applied after (v2.15.1), so a stray `STATUSLINE_THEME` line in `statusline.conf` can't clobber it (env-only knob; `colour_theme` is the conf key).
