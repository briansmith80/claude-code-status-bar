...
- **`refreshInterval` below the script's runtime blanks the whole bar** — Claude Code aborts an in-flight statusLine run when the next one starts, and an aborted run clears `statusLineText`. The script takes 1-3s per run on Windows (MSYS fork emulation), so `refreshInterval: 1` means every run is aborted and the bar never renders. Recommend `5`+ on Windows, `1` is fine on macOS/Linux (~100ms runtime).
