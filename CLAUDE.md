...
- **Colour themes via CLR_* variables** — All ANSI codes use theme variables set by `apply_theme()`. Eight built-in themes: default, nord, dracula, solarized, mono, tokyo-night, catppuccin, matrix (a monochrome digital-rain phosphor green, separated by brightness not hue). Supports NO_COLOR standard.
