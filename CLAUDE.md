...
- **Opt-in styling (v2.15.0)** — All default-off. `bar_gradient` colours the filled progress-bar cells along `CLR_RAMP0-3` (reuses the todo-bar stops + truecolour engine; no-op under mono/NO_COLOR). `nerd_font` swaps the scattered Unicode segment icons (`branch_icon`, `model_icon`, etc.) for Nerd Font glyphs defined once via `printf -v` (raw UTF-8 bytes — Bash 3.2-safe, no `\u`, fork-free, built only when enabled). `--demo [theme|all]` previews themes by re-invoking the script per theme with a `STATUSLINE_THEME` env override (applied after the conf) and the `--benchmark` canned payload — no recursion, no conf changes.
